...
<style>
@import url('https://fonts.googleapis.com/css2?family=Orbitron:wght@400;600;800;900&family=Rajdhani:wght@400;600;700&display=swap');
...
RHO_AIR     = 1.225  # kg/m3 - sea-level ISA air density
GRAVITY     = 9.81   # m/s2
...
“””
Euler-method simulation of a CO2 rocket car.
...
<div class="hero">
    <div class="hero-badge">F1 IN SCHOOLS &middot; ENGINEERING TOOL</div>
    <div class="hero-title">RACE <span>SIMULATOR</span></div>
    <div class="hero-sub">Physics-Based Performance Modelling &middot; Precise Controls &middot; Export Ready</div>
</div>
""", unsafe_allow_html=True)
...
color:#E8002D;letter-spacing:0.1em;padding:8px 0 4px;">
PARAMETERS
</div>
...
mass         = st.number_input("Car Mass (grams)",        30.0,   200.0,  55.0,   step=0.1,   format="%.1f")
drag_coeff   = st.number_input("Drag Coefficient (Cd)",   0.05,   1.5,    0.50,   step=0.01,  format="%.3f")
frontal_area = st.number_input("Frontal Area (m2)",        0.0005, 0.01,   0.0039, step=0.0001,format="%.4f")

st.markdown('<div class="sidebar-section">Propulsion</div>', unsafe_allow_html=True)
max_thrust        = st.number_input("Max Thrust Force (N)",     1.0,  40.0, 22.0, step=0.1,  format="%.1f")
thrust_decay_rate = st.number_input("Thrust Decay Rate (1/s)",  1.0,  50.0, 17.0, step=0.1,  format="%.1f")
...
<div style="font-size:0.7rem;color:#8888AA;line-height:1.9;">
    ...
    Air density: <b style="color:#E8E8F0;">{RHO_AIR} kg/m3</b> (ISA sea level)
</div>
""", unsafe_allow_html=True)
```
...
st.warning(
f”Car did not reach {track_length} m within {MAX_SIM_TIME} s. “
...
<div class="metric-row">
    ...
    <div class="metric-card accent-green">
        ...
        <div class="metric-sub">{mass:.1f} g &middot; Cd {drag_coeff:.3f}</div>
    </div>
</div>
""", unsafe_allow_html=True)
...
title         = dict(text=title, font=dict(family=OBT_FAM, size=11, color=”#E8E8F0”)),
...
with col_left:
    ...
    fig.add_trace(go.Scatter(
        x=t_vals, y=acc_vals, mode="lines", name="Accel (m/s2)",
...
df_display = pd.DataFrame({
    ...
    "Accel (m/s2)":  np.array(acc_vals[::downsample]).round(4),
})
...
with col_a:
    ...
    df_params = pd.DataFrame({
        "Parameter": [
            "Car Mass (g)", "Drag Coefficient (Cd)", "Frontal Area (m2)",
...
with col_b:
    ...
    df_perf = pd.DataFrame({
        "Metric": [
            ...
            "Peak Acceleration (m/s2)", "Peak G-Force",
...
def build_excel() -> bytes:
    ...
    df_p = pd.DataFrame({
        "Parameter": [
            "Car Mass (grams)", "Drag Coefficient (Cd)", "Frontal Area (m2)",
